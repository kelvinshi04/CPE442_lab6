...
inline float read_gray_int(__global const uchar* src,
                           int ix, int iy,
                           ...
                           int srcStep)
{
    ...
    // your exact integer grayscale:
    // gray = (18*b + 183*g + 54*r) >> 8;
    int gray = (18 * (int)b + 183 * (int)g + 54 * (int)r) >> 8;

    if (gray < 0)   gray = 0;
    if (gray > 255) gray = 255;

    return (float)gray;  // used as float in Sobel
}
    ...
    const int dstStep)
{
    ...
    float gCenter = read_gray_int(src, x, y, width, height, srcStep);

    // borders: just write grayscale (integer-based)
    ...
    // 3x3 neighborhood grayscale using your integer formula
    float g11 = read_gray_int(src, x-1, y-1, width, height, srcStep);
    float g12 = read_gray_int(src, x,   y-1, width, height, srcStep);
    float g13 = read_gray_int(src, x+1, y-1, width, height, srcStep);

    float g21 = read_gray_int(src, x-1, y,   width, height, srcStep);
    float g23 = read_gray_int(src, x+1, y,   width, height, srcStep);

    float g31 = read_gray_int(src, x-1, y+1, width, height, srcStep);
    float g32 = read_gray_int(src, x,   y+1, width, height, srcStep);
    float g33 = read_gray_int(src, x+1, y+1, width, height, srcStep);
...
int main(int argc, char** argv) {
    ...
    const char* srcStrs[] = { graySobelKernelSrcInt };
    size_t lengths[] = { strlen(graySobelKernelSrcInt) };
